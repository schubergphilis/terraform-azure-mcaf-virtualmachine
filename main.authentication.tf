...
resource "azurerm_key_vault_secret" "admin_password" {
  count = (((var.generate_admin_password_or_ssh_key == true) && (lower(var.os_type) == "windows") && var.generated_secrets_key_vault_secret_config != null) ||
  ((var.generate_admin_password_or_ssh_key == true) && (lower(var.os_type) == "linux") && (var.disable_password_authentication == false) && var.generated_secrets_key_vault_secret_config != null)) ? 1 : 0
...
resource "azurerm_key_vault_secret" "admin_ssh_key" {
  count = ((var.generate_admin_password_or_ssh_key == true) && (lower(var.os_type) == "linux") && (var.disable_password_authentication == true) && var.generated_secrets_key_vault_secret_config != null) ? 1 : 0

  key_vault_id    = coalesce(var.generated_secrets_key_vault_secret_config.key_vault_resource_id)
  name            = coalesce(var.generated_secrets_key_vault_secret_config.name, "${var.name}-${var.admin_username}-ssh-private-key")
